  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        if (data.products) {
          const femaleItems = data.products.filter((p: Product) => p.gender?.toLowerCase() === "female");
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] text-gray-800 py-20 lg:py-30">
      ...
      <div className="max-w-4xl mx-auto px-6 py-12 text-center">
        ...
        >
         <span className="text-[var(--primary-color)] italic">Women&apos;s  </span> Collection
        </motion.h1>
      </div>
      ...
      <div className="sticky top-[64px] z-50 w-full h-[55px] border-y border-gray-200 flex items-center justify-between bg-[#FDFBF7]/90 backdrop-blur-md px-4">
        ...
        <div className="flex items-center h-full">
          <button onClick={() => setShowSort(!showSort)} className="flex items-center gap-2 px-4 h-full border-l border-gray-200 text-[10px] font-bold uppercase tracking-widest">Sort</button>
          <button onClick={() => setShowFilters(!showFilters)} className="flex items-center gap-2 px-4 h-full border-l border-gray-200 text-[10px] font-bold uppercase tracking-widest">Filter</button>
          <button onClick={handleReset} className="px-4 border-l border-gray-200 text-gray-400 hover:text-red-500"><RotateCcw size={14} /></button>
        </div>
      </div>
                ...
                <motion.div layout key={product._id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="group">
                  ...
                  {gridCols !== 6 && (
                    <div className="mt-6 text-center">
                      ...
                      <p className="mt-2 text-[11px] font-mono font-bold text-[var(--primary-color)] uppercase">RS {product.totalPrice.toLocaleString()}</p>
                    </div>
                  )}
                </motion.div>
              ))}
