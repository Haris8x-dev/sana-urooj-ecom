...
export default function Shop() {
  ...
  // Set to a massive number by default so every product (even 10 Rs) shows up
  ...
  const filteredProducts = useMemo(() => {
    ...
    // Only apply the 5000-20000 constraint IF the user has moved the slider
    ...
    <div className="min-h-screen bg-[#FDFBF7] text-gray-800 py-20 lg:py-30">
      ...
      <div className="max-w-4xl mx-auto px-6 py-12 text-center">
        <motion.h1 
          ...
          className="text-3xl md:text-5xl -tracking-tight text-gray-900 font-serif mb-4"
        >
          <span>Shop at</span> <span className="italic text-[var(--primary-color)]">Urooj</span> Sana
        </motion.h1>
        <motion.p 
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          Discover pieces crafted for the bold, the elegant, and the modern soul.
        </motion.p>
      </div>
            ...
            >
              {[
                ...
                <button 
                  ...
                  className={`w-full text-left px-4 py-3 text-[9px] uppercase tracking-widest transition-colors ${sortBy === opt.val ? "bg-amber-50 text-amber-400 font-bold" : "hover:bg-gray-50"}`}
                >
                  {opt.label}
                </button>
              ))}
            ...
            >
              ...
              <div className="flex justify-between mt-4">
                ...
                <span className="text-[9px] font-bold text-amber-400 font-mono">
                  {maxPrice > 20000 ? "UNLIMITED" : `UP TO RS ${maxPrice.toLocaleString()}`}
                </span>
              </div>
      ...
      <div className="w-full py-12 px-6">
                      ...
                      <p className="mt-2 text-[11px] font-mono text-gray-500">
                        {product.badges?.saveRs?.active ? (
                          <span className="flex items-center justify-center gap-2">
                            <span className="line-through text-gray-300">RS {product.price.toLocaleString()}</span>
                            <span className="text-red-600 font-bold">RS {product.totalPrice.toLocaleString()}</span>
                          </span>
                        ) : (
        ...
        {!loading && filteredProducts.length === 0 && (
          <div className="text-center py-20">
            <p className="text-gray-400 uppercase tracking-widest text-xs">No products match your criteria.</p>
