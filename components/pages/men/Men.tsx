...
interface Product {
  ...
  images: ImageObject[];
  isSoldOut: boolean;
  gender: string; // Added gender field for filtering
}
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        if (data.products) {
          // FILTER: Only show products where gender is "male"
      ...
      <div className="w-full py-8 px-6">
        {loading ? (
          <div className="flex justify-center items-center h-64">
            <p className="text-gray-400 tracking-widest text-sm">LOADING MEN'S COLLECTION...</p>
          </div>
        ) : products.length === 0 ? (
          <div className="flex justify-center items-center h-64">
